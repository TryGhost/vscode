...
export const functionalDefinitions: HelperSchema[] = [
  {
    ...
    definition:
      'Loop helper designed for working with lists of posts. The `foreach` helper will output the content placed between its opening and closing tags `{{#foreach}}{{/foreach}}` once for each item in the collection passed to it.',
...
{{/foreach}}`,
...

  {
    ...
    usage: '{{#get "posts"}}{{/get}}',
    definition:
      '`{{#get}}` is a special block helper that makes a custom query to the Ghost API to fetch publicly available data. These requests are made server-side before your templates are rendered.',
...
{{/has}}`,
...

  {
    ...
    definition:
      'Test very simple conditionals. Like all block helpers, `{{#if}}` supports adding an `{{else}}` block or using `^` instead of `#` for negation',
    example: `{{#post}}
  {{#if featured}}
...
{{/post}}`,
...

  {
    ...
    definition:
      'Output content where the conditional is false. Use `{{else if ...}}`, `{{else match ...}}`, and so on to chain together multiple options like a switch statement.',
 ...
 {{/if}}`,
  ...
  {
    name: 'is',
    snippet: '{{#is ${1|home,index,post,page,tag,author,paged,private|}}}\n\t$2\n{{/is}}',
    type: HelperTypes['functional'],
    usage: '{{#is "contexts"}}',
    definition:
      'The `{{#is}}` helper allows you to check the context of the current route. For example, to check if this is the home page, a post, or a tag page. Use this to output different content based on the current context.',
...
{{/is}}`,
...

  {
    ...
    definition:
      '`{{#match}}` allows for simple comparisons and executes different template blocks depending on the outcome.',
...

  {
    ...
    definition:
      '`{`{#unless}}` is essentially the opposite of `{{#if}}`. If you want to test a negative conditional only: if you only need the `{{else}}` part of an `{{#if}}` statement, then `{{#unless}}` is what you need.',
